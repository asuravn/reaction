...
import xformCartItems from "./xformCartItems.js";
 ...
 */
function xformCartFulfillmentGroup(fulfillmentGroup, cart, context) {
  ...
  return {
    ...
    items: xformCartItems(context, cart.items.filter(({ _id }) => fulfillmentGroup.itemIds.includes(_id))),
    selectedFulfillmentOption,
    shippingAddress: fulfillmentGroup.address,
...
export default async function xformCartCheckout(collections, cart, context) {
  ...
  fulfillmentGroups = fulfillmentGroups.map((fulfillmentGroup) => xformCartFulfillmentGroup(fulfillmentGroup, cart, context));
